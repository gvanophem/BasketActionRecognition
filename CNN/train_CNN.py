...
def train_model(root_dir):
  """This method is responsible for training the model on the data present in root_dir

    Parameters
    ----------
    root_dir : str
      The path to the directory containing the data required for the training of the model.

    Returns
    -------
    Nothing but it trains the model on every match except for the ones contained in TEST_DIRECTORY
    """
  for field in os.listdir(root_dir):
    ...
    if(field != TEST_DIRECTORY):
      for game in os.listdir(os.path.join(root_dir, field)):
        ...
        #RETRIEVES DATA AND MANIPULATE IT
        ...
        #PREPROCESSING STEPS FOR WINDOW
        windows = []
        labels = []
